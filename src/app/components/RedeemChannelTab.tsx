...
import { HashchainProtocol, decodeContractError, HashchainProtocolABI } from "@hashchain/sdk";
...
const RedeemChannelTab: React.FC = () => {
  ...
  const redeemChannel = async () => {
    ...
    if (!finalHashValue || !finalHashValue.match(/^0x[a-fA-F0-9]{64}$/)) {
      setErrorMessage("Invalid final hash value (must be a 32-byte hex string).");
      setIsPending(false);
      return;
    }
    ...
    try {
      ...
      console.log("Initiating redeem transaction with:", { payer, finalHashValue, numberOfTokensUsed });
      const tx = await hashchainSDK.redeemChannel({
        payer : payer,
        tokenAddress: utils.getAddress("0x0000000000000000000000000000000000000000"),
      ...
      let errDecoded = decodeContractError(err, HashchainProtocolABI);
      console.log("Redeem Error:", errDecoded);
      console.error("Redeem Error:", err); // Temporary debug
      ...
      if (err.code === "INSUFFICIENT_FUNDS") {
        ...
        errorMsg = `Transaction Error: ${err.message || "Failed to redeem channel"}`;
      }
    ...
    <div>
      ...
      <div className="mb-4">
        <p className={`text-sm ${theme === "light" ? "text-gray-800" : "text-gray-200"}`}>
          Enter the details to redeem the payment channel.
          <br />
          Ensure the payer address, final hash value, and number of tokens used are correct.
        </p>
      </div>

      {/* Form Fields */}
      {[
        { label: "Payer Address", value: payer, setter: setPayer, placeholder: "Enter payer address" },
        { label: "Final Hash Value", value: finalHashValue, setter: setFinalHashValue, placeholder: "Enter final hash value (32-byte hex)" },
        { label: "Number of Tokens Used", value: numberOfTokensUsed, setter: setNumberOfTokensUsed, type: "number", placeholder: "Enter number of tokens used" },
      ].map(({ label, value, setter, type = "text", placeholder }) => (
